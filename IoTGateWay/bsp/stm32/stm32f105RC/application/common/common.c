...
{
    rt_uint8_t  temp;
    rt_uint16_t crc_word = 0xffff;

    while (length--)
    {
        temp = *p_buff++ ^ crc_word;
        crc_word >>= 8;
...
void BufferShow(rt_uint8_t *pData, rt_uint32_t len)
{
	if(0 < len && NULL != pData)
	{
		uint32_t line = (len>>4), i;

		if((len%16) > 0) line++;

		rt_memset(buf, 0x00, sizeof(buf));
		rt_memcpy(buf, pData, len);
		rt_kprintf("Data Length : %lu\r\n", len);
		rt_kprintf("=================================================\r\n");
		for(i=0; i<line; i++)
		{
			rt_kprintf(" %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\r\n",
